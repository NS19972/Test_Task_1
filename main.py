...
if __name__ == "__main__":
    ...
    if sst.train_button_clicked:
        ...
        if sst.test_button:  # Когда нажимаем на тестовую кнопку:
            ...
            st.markdown("Нужно сперва отметить, что распределение классов в обучающей и валидационной выборки не сбалансированы (см. круговую диаграмму распределения классов). "
                        "Примерно 54% от всех сотрудников попадают под одну и ту же категорию - соответственно, модель может предсказывать"
                        "эту категорию для всех сотрудников и получить 54% точности. Тем временем, самый мало представленный класс составляет всего 4.5% от датасета. "
                        "Данная разбалансировка в классах не только затрудняет обучения модели, но также и затрудняет грамотную оценку качества её работы. \n"
                        "Можно предположить, что приемлемая точность для модели - это 'значительно' выше 54%. Если же итоговая точность равна или ниже чем 54%, "
            ...
            st.markdown("Если рассмотреть задачу чисто с идеологической/теоретической точки зрения, можно прийти к мнению что такой вывод логичен. "
                        "Ведь всё таки, карьерные перспективы, которые мы пытаемся предсказать через искусственный интеллект, "
                        "в основном зависят от очень сложных человеческих факторов, амбиций, и людских отношений. "
                        ...
                        "внешних обстоятельств, и тд.) вряд ли даже возможно собрать и запаковать в простой .csv файл.")
            st.markdown("То есть, текущая задача скорее всего не решаема на практике с применением данного подхода. **Однако, "
                        "это не означает, что подобный подход нельзя использовать в других областях, чтобы решать другие, в чём то похожие задачи** "
                        "(например, классификация надёжных/ненадёжных клиентов в банковской сфере).")
            ...
            st.markdown("#### Вопрос для пользователя - какие реальные задачи можно решить с применением методов классификации в этом приложении?")
            with st.expander("Щёлкните сюда, чтобы увидеть ответ"):
                st.markdown("---")
                st.markdown("##### Ответ")
                st.markdown("В выводе №2, мы пришли к итогу что в данной задачи с предсказанием карьерного роста нету достаточно данных для решения задачи, "
                            "и что скорее всего, добыть истинные и годные данные вовсе невозможно или не практично. Исходя из этого, можно прийти к выводу "
                            "что технологии, которые предоставлены в этом приложении, желательно применять к другим задачам, где как раз есть возможность "
                            "добыть нужные и релевантные данные для обучения/работы модели.")

                st.markdown("Примерами таких задач могут быть: классификация надёжных клиентов в банковской сфере, классификация надёжных водителей/пилотов техники, "
                            "диагноз заболеваний по наличию симптомов, диагноз неисправности в технических препаратах, определение мошенников "
                            "в интернет-магазинах, обнаружение огнеопасных зон/зданий, и многое другое.")
